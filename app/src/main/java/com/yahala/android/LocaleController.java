...
import com.yahala.ui.FastDateFormat;
...
public class LocaleController {
    ...
    public LocaleController() {
        ...
        */
        ...
        localeInfo.pathToFile = null;
        sortedLanguages.add(1, localeInfo);
        languagesDict.put(localeInfo.shortName, localeInfo);
    ...
    public static String formatUserStatus(TLRPC.User user) {
        String lastSeenMessage = null;
        if (user.presence == null) {
            lastSeenMessage = LocaleController.getString("Offline", R.string.Offline);
        } else {
            if (user.presence.getType() == Presence.Type.available) {
                lastSeenMessage = LocaleController.getString("Online", R.string.Online);
            } else if (user.presence.getType() == Presence.Type.unavailable) {
                lastSeenMessage = LocaleController.getString("Offline", R.string.Offline);
            } else {
                lastSeenMessage = user.last_seen;
            }
        }
