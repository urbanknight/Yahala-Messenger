...
public class FileLoadOperation {
    ...
    public void start() {
        ...
        if (isLocalFile) {
            ...
            cacheFileFinal = new File(OSUtilities.getCacheDir(), fileNameFinal);
        }
        ...
        if ((exist = cacheFileFinal.exists()) && !ignoreCache) {
                    ...
                    try {
                        ...
                        if (needBitmapCreate) {
                            ...
                            if (filter != null) {
                                ...
                                w_filter = Float.parseFloat(args[0]) * OSUtilities.density;
                                h_filter = Float.parseFloat(args[1]) * OSUtilities.density;
            ...
            cacheFileTemp = new File(OSUtilities.getCacheDir(), fileNameTemp);
            if (cacheFileTemp.exists()) {
                downloadedBytes = (int) cacheFileTemp.length();
                nextDownloadOffset = downloadedBytes = downloadedBytes / 1024 * 1024;
            }
            if (fileNameIv != null) {
                cacheIvTemp = new File(OSUtilities.getCacheDir(), fileNameIv);
                try {
