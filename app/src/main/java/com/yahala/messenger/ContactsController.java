...
public class ContactsController {
    ...
    private ArrayList<String> delayedContactsUpdate = new ArrayList<String>();
    ...
    public void checkAppAccount() {
        ...
        /*  Account[] accounts = am.getAccountsByType("com.yahala.messenger.account");
        ...
        /*if (recreateAccount) {
            ...
            if (UserConfig.currentUser != null) {
                currentAccount = new Account(UserConfig.currentUser.phone, "com.yahala.messenger.account");
    ...
    public void performSyncPhoneBook(boolean first) {
        final HashMap<Integer, Contact> contactsMap = readContactsFromPhoneBook();
        //List<TLRPC.User> friend= XmppManager.getInstance().getFriends();
        HashMap<String, Contact> phoneBookFriends = new HashMap<String, Contact>();
        for (HashMap.Entry<Integer, Contact> entry : contactsMap.entrySet()) {
            Contact c = entry.getValue();

            for (String phone : c.shortPhones) {
                UserConfig.loadConfig();

                // PhoneFormat.getInstance().init(UserConfig.currentUser.);
                FileLog.e("Yahala", c.first_name + " " + phone);

                phoneBookFriends.put(phone, c);
                TLRPC.User user = new TLRPC.User();
                user.first_name = c.first_name;
                user.last_name = c.last_name;
                user.phone = 962 + c.phones.get(0);
                addContact(user);
                loadingContacts = false;
    ...
    public void loadContacts(boolean fromCache, boolean cacheEmpty) {
        ...
        if (fromCache) {
          ...
          /*  for (TLRPC.User u : XmppManager.getInstance().getFriends())
            {TLRPC.TL_contact c= new TLRPC.TL_contact();
                c.re
                users
            }
            processLoadedContacts( , XmppManager.getInstance().getFriends(), 0);*/
    ...
    public void processLoadedContacts(final ArrayList<TLRPC.TL_contact> contactsArr, final ArrayList<TLRPC.User> usersArr, final int from) {
        ...
        //contactsArr= XmppManager.getInstance().getFriends();
        FileLog.e("Contacts", "processLoadedContacts contacts: " + contactsArr.size() + ", users: " + usersArr.size());
        Utilities.RunOnUIThread(new Runnable() {
            @Override
            public void run() {
                for (TLRPC.User user : usersArr) {
                    if (from == 1) {
                        MessagesController.getInstance().users.putIfAbsent(user.jid, user);
                    } else {
                        MessagesController.getInstance().users.put(user.jid, user);
...
//                        }
    ...
    private void applyContactsUpdates(ArrayList<String> ids, ConcurrentHashMap<String, TLRPC.User> userDict, ArrayList<TLRPC.TL_contact> newC, ArrayList<String> contactsTD) {
        /*if (newC == null || contactsTD == null) {
            newC = new ArrayList<TLRPC.TL_contact>();
            contactsTD = new ArrayList<String>();
            for (String uid : ids) {
                if (Long.parseLong(uid)  > 0) {
                    TLRPC.TL_contact contact = new TLRPC.TL_contact();
                    contact.jid = uid;
                    newC.add(contact);
                } else if (Long.parseLong(uid) < 0) {
                    contactsTD.add(uid);
                }
            }
        }
        ...
        for (final String uid : contactsTD) {
            Utilities.globalQueue.postRunnable(new Runnable() {
                @Override
                public void run() {
        ...
        if (reloadContacts) {
            ...
            final ArrayList<String> contactsToDelete = contactsTD;
            Utilities.RunOnUIThread(new Runnable() {
                @Override
                public void run() {
                    ...
                    for (String uid : contactsToDelete) {
    ...
    public void processContactsUpdates(ArrayList<String> ids, ConcurrentHashMap<String, TLRPC.User> userDict) {
        ...
        for (String jid : ids) {
            if (Long.parseLong(jid) > 0) {
                TLRPC.TL_contact contact = new TLRPC.TL_contact();
                contact.jid = jid;
                newContacts.add(contact);
                if (!delayedContactsUpdate.isEmpty()) {
                    int idx = delayedContactsUpdate.indexOf(-jid);
                    if (idx != -1) {
                        delayedContactsUpdate.remove(idx);
                    }
                }
            } else if (Long.parseLong(jid) < 0) {
                contactsToDelete.add(-jid);
                if (!delayedContactsUpdate.isEmpty()) {
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.DATA2, "Yahala Profile");
    ...
    private void deleteContactFromPhoneBook(String jid) {
        ContentResolver contentResolver = ApplicationLoader.applicationContext.getContentResolver();
        synchronized (observerLock) {
            ignoreChanges = true;
        }
        try {
            Uri rawContactUri = ContactsContract.RawContacts.CONTENT_URI.buildUpon().appendQueryParameter(ContactsContract.CALLER_IS_SYNCADAPTER, "true").appendQueryParameter(ContactsContract.RawContacts.ACCOUNT_NAME, currentAccount.name).appendQueryParameter(ContactsContract.RawContacts.ACCOUNT_TYPE, currentAccount.type).build();
            int value = contentResolver.delete(rawContactUri, ContactsContract.RawContacts.SYNC2 + " = " + jid, null);
        } catch (Exception e) {
            FileLog.e("tmessages", e);
        }
        synchronized (observerLock) {
    ...
    public void addContact(TLRPC.User user) {
       /* if (user == null || user.phone == null) {
        ...
        /////////test
        final ArrayList<TLRPC.User> resUsers = XMPPManager.getInstance().getFriends(true);
        FileLog.e("Yahala ", "Start adding contacts");
        //MessagesStorage.getInstance().putUsersAndChats(resUsers, null, true, true);
        for (final TLRPC.User u : resUsers) {
            Utilities.globalQueue.postRunnable(new Runnable() {
                @Override
                public void run() {
                    FileLog.e("Yahala add contacts:", u.first_name + " " + u.phone);
        ...
        Utilities.RunOnUIThread(new Runnable() {
            @Override
            public void run() {
                for (TLRPC.User u : resUsers) {
                    MessagesController.getInstance().users.put(u.jid, u);
