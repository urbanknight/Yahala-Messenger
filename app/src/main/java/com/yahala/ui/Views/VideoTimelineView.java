...
public class VideoTimelineView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        int width = getMeasuredWidth() - OSUtilities.dp(12);
        int startX = (int) (width * progressLeft) + OSUtilities.dp(3);
        int endX = (int) (width * progressRight) + OSUtilities.dp(9);

        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            int additionWidth = OSUtilities.dp(12);
            ...
            if (pressedLeft) {
                ...
                if (startX < OSUtilities.dp(3)) {
                    startX = OSUtilities.dp(3);
                } else if (startX > endX - OSUtilities.dp(6)) {
                    startX = endX - OSUtilities.dp(6);
                }
                progressLeft = (float) (startX - OSUtilities.dp(3)) / (float) width;
                if (delegate != null) {
                ...
                endX = (int) (x - pressDx);
                if (endX < startX + OSUtilities.dp(6)) {
                    endX = startX + OSUtilities.dp(6);
                } else if (endX > width + OSUtilities.dp(9)) {
                    endX = width + OSUtilities.dp(9);
                }
                progressRight = (float) (endX - OSUtilities.dp(9)) / (float) width;
                if (delegate != null) {
    ...
    private void reloadFrames(int frameNum) {
        ...
        if (frameNum == 0) {
            frameHeight = getMeasuredHeight() - OSUtilities.dp(4);
    ...
    protected void onDraw(Canvas canvas) {
        if (frames.isEmpty() && currentTask == null) {
            ...
            for (Bitmap bitmap : frames) {
                if (bitmap != null) {
                    canvas.drawBitmap(bitmap, offset * frameWidth, OSUtilities.dp(2), null);
                }
                offset++;
            }
        }
        ...
        canvas.drawRect(startX, 0, startX + OSUtilities.dp(6), getMeasuredHeight(), paint);
        canvas.drawRect(endX + OSUtilities.dp(6), 0, endX + OSUtilities.dp(12), getMeasuredHeight(), paint);
        canvas.drawRect(startX + OSUtilities.dp(6), OSUtilities.dp(4), endX + OSUtilities.dp(6), getMeasuredHeight() - OSUtilities.dp(4), paint2);
        canvas.drawRect(startX + OSUtilities.dp(6), OSUtilities.dp(2), endX + OSUtilities.dp(6), OSUtilities.dp(4), paint);
        canvas.drawRect(startX + OSUtilities.dp(6), getMeasuredHeight() - OSUtilities.dp(4), endX + OSUtilities.dp(6), getMeasuredHeight() - OSUtilities.dp(2), paint);
    }
}
