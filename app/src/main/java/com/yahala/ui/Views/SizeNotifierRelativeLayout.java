...
public class SizeNotifierRelativeLayout extends RelativeLayout {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (delegate != null) {
            int usableViewHeight = this.getRootView().getHeight() - OSUtilities.statusBarHeight;
    ...
    protected void onDraw(Canvas canvas) {
        if (backgroundDrawable != null) {
            float scaleX = (float) OSUtilities.displaySize.x / (float) backgroundDrawable.getIntrinsicWidth();
            float scaleY = (float) OSUtilities.displaySize.y / (float) backgroundDrawable.getIntrinsicHeight();
            float scale = scaleX < scaleY ? scaleY : scaleX;
            int width = (int) Math.ceil(backgroundDrawable.getIntrinsicWidth() * scale);
            int height = (int) Math.ceil(backgroundDrawable.getIntrinsicHeight() * scale);
            int x = (OSUtilities.displaySize.x - width) / 2;
            int y = (OSUtilities.displaySize.y - height) / 2;
