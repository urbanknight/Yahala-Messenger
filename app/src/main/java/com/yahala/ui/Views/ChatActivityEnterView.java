...
public class ChatActivityEnterView implements NotificationCenter.NotificationCenterDelegate, SizeNotifierRelativeLayout.SizeNotifierRelativeLayoutDelegate {
    ...
    private EmojiViewExtra emojiView;
    ...
    public void setContainerView(Activity activity, View containerView) {
        ...
        stickersButton = (ImageView) containerView.findViewById(R.id.chat_sticker_button);
        stickersButton.setVisibility(View.INVISIBLE);
        ...
        stickersButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (emojiPopup == null) {
                    showEmojiPopup(true, true);
                } else if (emojiPopup != null && emojiPopup.getContentView() instanceof EmojiViewExtra) {
                    showEmojiPopup(true, true);
                } else {
                    showEmojiPopup(!emojiPopup.isShowing(), true);
                }
            }
        });
        emojiButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (emojiPopup == null) {
                    showEmojiPopup(true);
                } else if (emojiPopup != null && emojiPopup.getContentView() instanceof StickersView) {
                    showEmojiPopup(true);
                } else {
                ...
                if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (android.os.Build.VERSION.SDK_INT > 13) {
                        ...
                        if (startedDraggingX != -1) {
                            ...
                            params.leftMargin = OSUtilities.dp(30) + (int) dist;
                        ...
                        if (x <= slideText.getX() + slideText.getWidth() + OSUtilities.dp(30)) {
                            if (startedDraggingX == -1) {
                                startedDraggingX = x;
                                distCanMove = (recordPanel.getMeasuredWidth() - slideText.getMeasuredWidth() - OSUtilities.dp(48)) / 2.0f;
                                if (distCanMove <= 0) {
                                    distCanMove = OSUtilities.dp(80);
                                } else if (distCanMove > OSUtilities.dp(80)) {
                                    distCanMove = OSUtilities.dp(80);
                                }
                            }
                        }
                        if (params.leftMargin > OSUtilities.dp(30)) {
                            params.leftMargin = OSUtilities.dp(30);
                            slideText.setLayoutParams(params);
        ...
        messsageEditText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence charSequence, int i, int i2, int i3) {
                ...
                final TLRPC.User currentUser = ContactsController.getInstance().friendsDict.get(dialog_id);
                if (message.length() != 0 && lastTypingTimeSend < System.currentTimeMillis() - 5000 /*&& !ignoreTextChange*/) {
                    ...
                    XMPPManager.getInstance().updateChatState(currentUser.jid, ChatState.composing);
                    new Timer().schedule(new TimerTask() {
                        @Override
                        public void run() {
                            XMPPManager.getInstance().updateChatState(currentUser.jid, ChatState.active);
                            FileLog.e("updateChatState", " ChatState.active");
                        }
                    }, 5000);
            ...
            public void afterTextChanged(Editable editable) {
                FileLog.e("afterTextChanged", "afterTextChanged");
                ...
                while (true) {
                    if (i >= j) {
                        EmojiManager.getInstance().replaceEmoji(editable, messsageEditText.getPaint().getFontMetricsInt(), OSUtilities.dp(32));
                        return;
                    }
    ...
    public boolean processSendingText(String text) {
        ...
        if (text.length() != 0) {
            ...
            XMPPManager.getInstance().sendMessage(text, dialog_id, id);
            return true;
        }
        return false;
    }
    ...
    private void updateAudioRecordIntefrace() {
        if (recordingAudio) {
            ...
            if (android.os.Build.VERSION.SDK_INT > 13) {
                ...
                slideText.setLayoutParams(params);
                slideText.setAlpha(1);
                recordPanel.setX(OSUtilities.displaySize.x);
            ...
            if (android.os.Build.VERSION.SDK_INT > 13) {
                ...
                }).setDuration(300).translationX(OSUtilities.displaySize.x).start();
            } else {
                recordPanel.setVisibility(View.GONE);
            }
        }
    ...
    private void showEmojiPopup(boolean show) {
        showEmojiPopup(show, false);
    }

    private void showEmojiPopup(boolean show, boolean sticker) {
        ...
        if (show) {
            if (emojiPopup != null /*&& (emojiPopup.getContentView() instanceof EmojiViewExtra ||
                    emojiPopup.getContentView() instanceof StickersView)*/) {
                emojiPopup.dismiss();
                emojiPopup = null;
                FileLog.e("emojiPopup", "!emojiPopup != null");
            }
            if (emojiPopup == null && !sticker) {
                FileLog.e("emojiPopup", "!sticker");
                createEmojiPopup();
            } else if (emojiPopup == null && sticker) {
                FileLog.e("emojiPopup", "sticker");
                createStickersPopup();
            }
            ...
            if (keyboardHeight <= 0) {
                keyboardHeight = ApplicationLoader.applicationContext.getSharedPreferences("emoji", 0).getInt("kbd_height", OSUtilities.dp(200));
            }
            if (keyboardHeightLand <= 0) {
                keyboardHeightLand = ApplicationLoader.applicationContext.getSharedPreferences("emoji", 0).getInt("kbd_height_land3", OSUtilities.dp(200));
            }
    ...
    private void createEmojiPopup() {
        ...
        emojiView = new EmojiViewExtra(parentActivity);
        emojiView.setListener(new EmojiViewExtra.Listener() {
            public void onBackspace() {
                messsageEditText.dispatchKeyEvent(new KeyEvent(0, 67));
            }

            public void onEmojiSelected(Emoji t) {
                String paramAnonymousString = ':' + t.name + ':';


                int i = messsageEditText.getSelectionEnd();
                CharSequence localCharSequence = EmojiManager.getInstance().replaceEmoji(paramAnonymousString, messsageEditText.getPaint().getFontMetricsInt(), OSUtilities.dp(32));
    ...
    private void createStickersPopup() {
        if (parentActivity == null) {
            return;
        }
        stickersView = new StickersView(parentActivity);
        stickersView.setListener(new StickersView.Listener() {
            public void onBackspace() {
                messsageEditText.dispatchKeyEvent(new KeyEvent(0, 67));
            }

            public void onStickerSelected(Sticker t) {
              /*  String paramAnonymousString=':'+t.name+':';
        ...
        emojiPopup = new PopupWindow(stickersView);
    }
    ...
    public void didReceivedNotification(int id, Object... args) {
        if (id == 999) {
            ...
            if (messsageEditText != null && messsageEditText.isFocused()) {
                OSUtilities.hideKeyboard(messsageEditText);
            }
