...
public class ActionBarLayer extends FrameLayout {
    ...
    private void positionBackImage(int height) {
        if (backButtonImageView != null) {
            ...
            layoutParams.gravity = Gravity.TOP | Gravity.LEFT;
            layoutParams.setMargins(OSUtilities.dp(3), (height - backButtonImageView.getDrawable().getIntrinsicHeight()) / 2, 0, 0);
    ...
    private void positionTitle(int width, int height) {
        int offset = OSUtilities.dp(2);
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            offset = OSUtilities.dp(1);
        }
        int maxTextWidth = 0;
        ...
        if (logoImageView == null || logoImageView.getVisibility() == GONE) {
            ...
            if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
                x = OSUtilities.dp(22) + (int) (logoImageView.getDrawable().getIntrinsicWidth() / 1.3f);
            } else {
                x = OSUtilities.dp(22) + logoImageView.getDrawable().getIntrinsicWidth();
            }
        }

        if (menu != null) {
            maxTextWidth = Math.min(maxTextWidth, width - menu.getMeasuredWidth() - OSUtilities.dp(16));
        }
        ...
        layoutParams1.width = x + maxTextWidth + (isSearchFieldVisible ? 0 : OSUtilities.dp(6));
    ...
    public void positionMenu(int width, int height) {
        ...
        layoutParams.leftMargin = isSearchFieldVisible ? OSUtilities.dp(26) + logoImageView.getDrawable().getIntrinsicWidth() : 0;
...
//        actionMode.setBackgroundResource(R.drawable.editheader);
    ...
    protected void onSearchFieldVisibilityChanged(boolean visible) {
        ...
        backButtonFrameLayout.setPadding(0, 0, visible ? 0 : OSUtilities.dp(4), 0);
        if (visible) {
