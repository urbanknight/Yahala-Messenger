...
import com.yahala.ui.Views.VideoSeekBarView;
import com.yahala.ui.Views.VideoTimelineView;
...
public class VideoEditorActivity extends BaseFragment implements SurfaceHolder.Callback {
    ...
    private void fixVideoSize() {
        ...
        if (!Utilities.isTablet(getParentActivity()) && getParentActivity().getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            viewHeight = OSUtilities.displaySize.y - OSUtilities.statusBarHeight - OSUtilities.dp(40);
        } else {
            viewHeight = OSUtilities.displaySize.y - OSUtilities.statusBarHeight - OSUtilities.dp(48);
        }

        int width = 0;
        int height = 0;
        if (getParentActivity().getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            width = OSUtilities.displaySize.x - OSUtilities.displaySize.x / 2 - OSUtilities.dp(24);
            height = viewHeight - OSUtilities.dp(32);
        } else {
            width = OSUtilities.displaySize.x;
            height = viewHeight - OSUtilities.dp(176);
        }
        ...
        originalSizeTextView.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                if (getParentActivity().getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
                    ...
                    layoutParams.topMargin = OSUtilities.dp(16);
                    layoutParams.bottomMargin = OSUtilities.dp(16);
                    layoutParams.width = OSUtilities.displaySize.x / 2 - OSUtilities.dp(24);
                    layoutParams.leftMargin = OSUtilities.dp(16);
                    ...
                    layoutParams.width = OSUtilities.displaySize.x / 2 - OSUtilities.dp(24);
                    layoutParams.leftMargin = OSUtilities.displaySize.x / 2 + OSUtilities.dp(8);
                    layoutParams.rightMargin = OSUtilities.dp(16);
                    layoutParams.topMargin = OSUtilities.dp(16);
                    textContainerView.setLayoutParams(layoutParams);
                } else {
                    FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) videoContainerView.getLayoutParams();
                    layoutParams.topMargin = OSUtilities.dp(16);
                    layoutParams.bottomMargin = OSUtilities.dp(160);
                    layoutParams.width = FrameLayout.LayoutParams.MATCH_PARENT;
                    layoutParams.leftMargin = 0;
                    videoContainerView.setLayoutParams(layoutParams);

                    layoutParams = (FrameLayout.LayoutParams) textContainerView.getLayoutParams();
                    layoutParams.height = OSUtilities.dp(143);
                    layoutParams.width = FrameLayout.LayoutParams.MATCH_PARENT;
                    layoutParams.leftMargin = 0;
