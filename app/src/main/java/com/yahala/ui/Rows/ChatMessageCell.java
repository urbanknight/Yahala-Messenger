...
public class ChatMessageCell extends ChatBaseCell {
    ...
    public void setMessageObject(MessageObject messageObject) {
        if (currentMessageObject != messageObject || isUserDataChanged()) {
            ...
            if (isChat && !messageObject.isOut()) {
                maxWidth = OSUtilities.displaySize.x - OSUtilities.dp(122);
                drawName = true;
            } else {
                maxWidth = OSUtilities.displaySize.x - OSUtilities.dp(80);
                drawName = false;
            }
            ...
            totalHeight = messageObject.textHeight + OSUtilities.dpf(19.5f) + namesOffset;

            int maxChildWidth = Math.max(backgroundWidth, nameWidth);
            maxChildWidth = Math.max(maxChildWidth, forwardedNameWidth);

            int timeMore = timeWidth + OSUtilities.dp(6);
            if (messageObject.isOut()) {
                timeMore += OSUtilities.dpf(20.5f);
            }

            if (maxWidth - messageObject.lastLineWidth < timeMore) {
                totalHeight += OSUtilities.dp(14);
                backgroundWidth = Math.max(maxChildWidth, messageObject.lastLineWidth) + OSUtilities.dp(29);
            } else {
                int diff = maxChildWidth - messageObject.lastLineWidth;
                if (diff >= 0 && diff <= timeMore) {
                    backgroundWidth = maxChildWidth + timeMore - diff + OSUtilities.dp(29);
                } else {
                    backgroundWidth = Math.max(maxChildWidth, messageObject.lastLineWidth + timeMore) + OSUtilities.dp(29);
                }
            }
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (currentMessageObject.isOut()) {
            textX = layoutWidth - backgroundWidth + OSUtilities.dp(10);
            textY = OSUtilities.dp(10) + namesOffset;
        } else {
            textX = OSUtilities.dp(19) + (isChat ? OSUtilities.dp(52) : 0);
            textY = OSUtilities.dp(10) + namesOffset;
        }
    }
