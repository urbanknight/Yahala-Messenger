...
import com.yahala.ui.Views.GifDrawable;
import com.yahala.ui.PhotoViewer;
...
public class ChatMediaCell extends ChatBaseCell implements MediaController.FileDownloadProgressListener {
    ...
    public ChatMediaCell(Context context) {
        ...
        if (placeholderInDrawable == null) {
            ...
            infoPaint.setTextSize(OSUtilities.dp(12));
        }
    ...
    private void didPressedButton() {
        if (buttonState == 0) {
            if (currentMessageObject.type == 1) {
                ...
                FileLoader.getInstance().loadFile(null, null, currentMessageObject.messageOwner.tl_message.media.document, null);
            ...
            if (currentMessageObject.isOut() && currentMessageObject.messageOwner.getSend_state() == MessagesController.MESSAGE_SEND_STATE_SENDING) {
                if (delegate != null) {
                ...
                if (currentMessageObject.type == 1) {
                    ...
                    FileLoader.getInstance().cancelLoadFile(null, null, currentMessageObject.messageOwner.tl_message.media.document, null);
    ...
    private boolean isPhotoDataChanged(MessageObject object) {
        if (object.type == 4) {
            ...
            double lat = object.messageOwner.tl_message.media.geo.lat;
            double lon = object.messageOwner.tl_message.media.geo._long;
            String url = String.format(Locale.US, "https://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=13&size=100x100&maptype=roadmap&scale=%d&markers=color:red|size:big|%f,%f&sensor=false", lat, lon, Math.min(2, (int) Math.ceil(OSUtilities.density)), lat, lon);
    ...
    public void setMessageObject(MessageObject messageObject) {
        if (currentMessageObject != messageObject || isPhotoDataChanged(messageObject) || isUserDataChanged()) {
            ...
            if (messageObject.type == 8) {
                ...
                String str = Utilities.formatFileSize(messageObject.messageOwner.tl_message.media.document.size);
                ...
                int duration = messageObject.messageOwner.tl_message.media.video.duration;
                int minutes = duration / 60;
                int seconds = duration - minutes * 60;
                String str = String.format("%d:%02d, %s", minutes, seconds, Utilities.formatFileSize(messageObject.messageOwner.tl_message.media.video.size));
                if (currentInfoString == null || !currentInfoString.equals(str)) {
                    currentInfoString = str;
                    infoOffset = videoIconDrawable.getIntrinsicWidth() + OSUtilities.dp(4);
                    infoWidth = (int) Math.ceil(infoPaint.measureText(currentInfoString));
                    infoLayout = new StaticLayout(currentInfoString, infoPaint, infoWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                }
            } else {
            ...
            if (messageObject.type == 4) {
                photoWidth = OSUtilities.dp(100);
                photoHeight = OSUtilities.dp(100);
                backgroundWidth = photoWidth + OSUtilities.dp(12);

                double lat = messageObject.messageOwner.tl_message.media.geo.lat;
                double lon = messageObject.messageOwner.tl_message.media.geo._long;
                currentUrl = String.format(Locale.US, "https://maps.googleapis.com/maps/api/staticmap?center=%f,%f&zoom=13&size=100x100&maptype=roadmap&scale=%d&markers=color:red|size:big|%f,%f&sensor=false", lat, lon, Math.min(2, (int) Math.ceil(OSUtilities.density)), lat, lon);
                photoImage.setImage(currentUrl, null, messageObject.messageOwner.getOut() == 1 ? placeholderOutDrawable : placeholderInDrawable);
            } else {
                photoWidth = (int) (Math.min(OSUtilities.displaySize.x, OSUtilities.displaySize.y) * 0.7f);
                photoHeight = photoWidth + OSUtilities.dp(100);
                ...
                if (currentPhotoObject != null) {
                    ...
                    if (w == 0) {
                        if (messageObject.type == 3) {
                            w = infoWidth + infoOffset + OSUtilities.dp(16);
                        } else {
                            w = OSUtilities.dp(100);
                        }
                    }
                    if (h == 0) {
                        h = OSUtilities.dp(100);
                    }
                    if (h > photoHeight) {
                        float scale2 = h;
                        h = photoHeight;
                        scale2 /= h;
                        w = (int) (w / scale2);
                    } else if (h < OSUtilities.dp(120)) {
                        h = OSUtilities.dp(120);
                    ...
                    int timeWidthTotal = timeWidth + OSUtilities.dp(14 + (currentMessageObject.messageOwner.getOut() == 1 ? 20 : 0));
                    if (w < timeWidthTotal) {
                        w = timeWidthTotal;
                    }
                    ...
                    photoWidth = w;
                    photoHeight = h;
                    backgroundWidth = w + OSUtilities.dp(12);
                    currentPhotoFilter = String.format(Locale.US, "%d_%d", (int) (w / OSUtilities.density), (int) (h / OSUtilities.density));
                    ...
                    if (currentPhotoObject.image != null) {
                        ...
                        if (photoExist || downloadPhotos == 0 || downloadPhotos == 2 && ConnectionsManager.isConnectedToWiFi()) {
                            if (messageObject.imagePreview != null) {
                                ...
                                photoImage.setImage(currentPhotoObject.photoOwner.location, currentPhotoFilter, messageObject.messageOwner.getOut() == 1 ? placeholderOutDrawable : placeholderInDrawable, currentPhotoObject.photoOwner.size);
                            }
                        } else {
                            photoNotSet = true;
                            if (messageObject.imagePreview != null) {
                                ...
                                photoImage.setImageBitmap(messageObject.messageOwner.getOut() == 1 ? placeholderOutDrawable : placeholderInDrawable);
                            }
                        }
                    }
                } else {
    ...
    public void updateButtonState() {
        ...
        if (currentMessageObject.type == 1) {
            ...
            cacheFile = new File(OSUtilities.getCacheDir(), fileName);
            ...
            if (currentMessageObject.messageOwner.tl_message.attachPath != null && currentMessageObject.messageOwner.tl_message.attachPath.length() != 0) {
                ...
                if (f.exists()) {
                    fileName = currentMessageObject.messageOwner.tl_message.attachPath;
                    cacheFile = f;
                }
            }
            if (fileName == null) {
        ...
        if (currentMessageObject.messageOwner.getOut() == 1 && currentMessageObject.messageOwner.getSend_state() == MessagesController.MESSAGE_SEND_STATE_SENDING) {
            if (currentMessageObject.messageOwner.tl_message.attachPath != null) {
                MediaController.getInstance().addLoadingFileObserver(currentMessageObject.messageOwner.tl_message.attachPath, this);
                progressVisible = true;
                buttonState = 1;
                Float progress = FileLoader.getInstance().fileProgresses.get(currentMessageObject.messageOwner.tl_message.attachPath);
                if (progress != null) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(MeasureSpec.getSize(widthMeasureSpec), photoHeight + OSUtilities.dp(14));
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (currentMessageObject.isOut()) {
            photoImage.imageX = layoutWidth - backgroundWidth - OSUtilities.dp(3);
        } else {
            if (isChat) {
                photoImage.imageX = OSUtilities.dp(67);
            } else {
                photoImage.imageX = OSUtilities.dp(15);
            }
        }
        photoImage.imageY = OSUtilities.dp(7);
        photoImage.imageW = photoWidth;
        photoImage.imageH = photoHeight;

        progressView.width = timeX - photoImage.imageX - OSUtilities.dpf(23.0f);
        progressView.height = OSUtilities.dp(3);
        progressView.progressHeight = OSUtilities.dp(3);

        int size = OSUtilities.dp(44);
    ...
    protected void onAfterBackgroundDraw(Canvas canvas) {
        ...
        if (progressVisible) {
            setDrawableBounds(mediaBackgroundDrawable, photoImage.imageX + OSUtilities.dp(4), layoutHeight - OSUtilities.dpf(27.5f), progressView.width + OSUtilities.dp(12), OSUtilities.dpf(16.5f));
            mediaBackgroundDrawable.draw(canvas);

            canvas.save();
            canvas.translate(photoImage.imageX + OSUtilities.dp(10), layoutHeight - OSUtilities.dpf(21.0f));
        ...
        if (infoLayout != null && (buttonState == 1 || buttonState == 0 || buttonState == 3)) {
            setDrawableBounds(mediaBackgroundDrawable, photoImage.imageX + OSUtilities.dp(4), photoImage.imageY + OSUtilities.dp(4), infoWidth + OSUtilities.dp(8) + infoOffset, OSUtilities.dpf(16.5f));
            mediaBackgroundDrawable.draw(canvas);

            if (currentMessageObject.type == 3) {
                setDrawableBounds(videoIconDrawable, photoImage.imageX + OSUtilities.dp(8), photoImage.imageY + OSUtilities.dpf(7.5f));
                videoIconDrawable.draw(canvas);
            }

            canvas.save();
            canvas.translate(photoImage.imageX + OSUtilities.dp(8) + infoOffset, photoImage.imageY + OSUtilities.dpf(5.5f));
    ...
    public void onSuccessDownload(String fileName) {
        updateButtonState();
        if (currentMessageObject.type == 8 && lastDownloadedGifMessage != null && lastDownloadedGifMessage.messageOwner.getId() == currentMessageObject.messageOwner.id && buttonState == 2) {
            didPressedButton();
        }
        if (photoNotSet) {
