...
public class ChatBaseCell extends BaseCell {
    ...
    protected boolean isUserDataChanged() {
        ...
        TLRPC.User newUser = ContactsController.getInstance().friendsDict.get(currentMessageObject.messageOwner.getJid());
        ...
        newNameString = null;
        if (newUser != null && drawForwardedName && currentMessageObject.messageOwner.tl_message instanceof TLRPC.TL_messageForwarded) {
            newNameString = Utilities.formatName(newUser.first_name, newUser.last_name);
        }*/
    ...
    protected int getMaxNameWidth() {
        return backgroundWidth - OSUtilities.dp(8);
    }
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (delegate == null || delegate.canPerformActions()) {
                if (isAvatarVisible && x >= avatarImage.imageX && x <= avatarImage.imageX + avatarImage.imageW && y >= avatarImage.imageY && y <= avatarImage.imageY + avatarImage.imageH) {
                    ...
                    if (x >= forwardNameX && x <= forwardNameX + forwardedNameWidth && y >= forwardNameY && y <= forwardNameY + OSUtilities.dp(32)) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isAvatarVisible) {
            avatarImage.draw(canvas, OSUtilities.dp(6), layoutHeight - OSUtilities.dp(45), OSUtilities.dp(42), OSUtilities.dp(42));
        }
        ...
        if (currentMessageObject.messageOwner.getOut() == 1) {
            ...
            setDrawableBounds(currentBackgroundDrawable, layoutWidth - backgroundWidth - (!media ? 0 : OSUtilities.dp(9)), OSUtilities.dp(1), backgroundWidth, layoutHeight - OSUtilities.dp(2));
        } else {
            if (isPressed() && isCheckPressed || !isCheckPressed && isPressed) {
                if (!media) {
            ...
            if (isChat) {
                setDrawableBounds(currentBackgroundDrawable, OSUtilities.dp(52 + (!media ? 0 : 9)), OSUtilities.dp(1), backgroundWidth, layoutHeight - OSUtilities.dp(2));
            } else {
                setDrawableBounds(currentBackgroundDrawable, (!media ? 0 : OSUtilities.dp(9)), OSUtilities.dp(3), backgroundWidth, layoutHeight - OSUtilities.dp(4));
            }
        }
        ...
        if (drawName && nameLayout != null) {
            canvas.save();
            canvas.translate(currentBackgroundDrawable.getBounds().left + OSUtilities.dp(19) - nameOffsetX, OSUtilities.dp(10));
        ...
        if (drawForwardedName && forwardedNameLayout != null) {
            ...
            if (currentMessageObject.messageOwner.getOut() == 1) {
                ...
                forwardNameX = currentBackgroundDrawable.getBounds().left + OSUtilities.dp(10);
                forwardNameY = OSUtilities.dp(10 + (drawName ? 18 : 0));
            } else {
                forwardNamePaint.setColor(0xff006fc8);
                forwardNameX = currentBackgroundDrawable.getBounds().left + OSUtilities.dp(19);
                forwardNameY = OSUtilities.dp(10 + (drawName ? 18 : 0));
            }
        ...
        if (drawTime) {
            if (media) {
                setDrawableBounds(mediaBackgroundDrawable, timeX - OSUtilities.dp(3), layoutHeight - OSUtilities.dpf(27.5f), timeWidth + OSUtilities.dp(6 + (currentMessageObject.messageOwner.getOut() == 1 ? 20 : 0)), OSUtilities.dpf(16.5f));
                mediaBackgroundDrawable.draw(canvas);

                canvas.save();
                canvas.translate(timeX, layoutHeight - OSUtilities.dpf(12.0f) - timeLayout.getHeight());
                timeLayout.draw(canvas);
                canvas.restore();
            } else {
                canvas.save();
                canvas.translate(timeX, layoutHeight - OSUtilities.dpf(6.5f) - timeLayout.getHeight());
                timeLayout.draw(canvas);
                canvas.restore();
            }
            ...
            if (currentMessageObject.isOut()) {
                ...
                if (currentMessageObject.messageOwner.getSend_state() == XMPPManager.MESSAGE_SEND_STATE_SENDING) {
                    drawCheck1 = false;
                    drawCheck2 = false;
                    drawClock = true;
                    drawError = false;
                } else if (currentMessageObject.messageOwner.getSend_state() == XMPPManager.MESSAGE_SEND_STATE_SEND_ERROR) {
                    drawCheck1 = false;
                    drawCheck2 = false;
                    drawClock = false;
                    drawError = true;
                } else if (currentMessageObject.messageOwner.getSend_state() == XMPPManager.MESSAGE_SEND_STATE_SENT) {
                    ...
                    drawClock = false;
                    drawError = false;
                } else if (currentMessageObject.messageOwner.getSend_state() == XMPPManager.MESSAGE_SEND_STATE_AKN) {
                ...
                if (drawClock) {
                    if (!media) {
                        setDrawableBounds(clockDrawable, layoutWidth - OSUtilities.dpf(18.5f) - clockDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(8.5f) - clockDrawable.getIntrinsicHeight());
                        clockDrawable.draw(canvas);
                    } else {
                        setDrawableBounds(clockMediaDrawable, layoutWidth - OSUtilities.dpf(22.0f) - clockMediaDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(13.0f) - clockMediaDrawable.getIntrinsicHeight());
                ...
                if (drawCheck2) {
                    if (!media) {
                        if (drawCheck1) {
                            setDrawableBounds(checkDrawable, layoutWidth - OSUtilities.dpf(22.5f) - checkDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(8.5f) - checkDrawable.getIntrinsicHeight());
                        } else {
                            setDrawableBounds(checkDrawable, layoutWidth - OSUtilities.dpf(18.5f) - checkDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(8.5f) - checkDrawable.getIntrinsicHeight());
                        }
                        checkDrawable.draw(canvas);
                    } else {
                        if (drawCheck1) {
                            setDrawableBounds(checkMediaDrawable, layoutWidth - OSUtilities.dpf(26.0f) - checkMediaDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(13.0f) - checkMediaDrawable.getIntrinsicHeight());
                        } else {
                            setDrawableBounds(checkMediaDrawable, layoutWidth - OSUtilities.dpf(22.0f) - checkMediaDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(13.0f) - checkMediaDrawable.getIntrinsicHeight());
                        }
                ...
                if (drawCheck1) {
                    if (!media) {
                        setDrawableBounds(halfCheckDrawable, layoutWidth - OSUtilities.dp(18) - halfCheckDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(8.5f) - halfCheckDrawable.getIntrinsicHeight());
                        halfCheckDrawable.draw(canvas);
                    } else {
                        setDrawableBounds(halfCheckMediaDrawable, layoutWidth - OSUtilities.dpf(20.5f) - halfCheckMediaDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(13.0f) - halfCheckMediaDrawable.getIntrinsicHeight());
                        halfCheckMediaDrawable.draw(canvas);
                    }
                }
                if (drawError) {
                    if (!media) {
                        setDrawableBounds(errorDrawable, layoutWidth - OSUtilities.dp(18) - errorDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(6.5f) - errorDrawable.getIntrinsicHeight());
                        errorDrawable.draw(canvas);
                    } else {
                        setDrawableBounds(errorDrawable, layoutWidth - OSUtilities.dpf(20.5f) - errorDrawable.getIntrinsicWidth(), layoutHeight - OSUtilities.dpf(12.5f) - errorDrawable.getIntrinsicHeight());
                        errorDrawable.draw(canvas);
                    }
                }
            }
